...
export function addOrEditTodo(type, todoId = "") {
  ...
  todoPopupSaveButton.addEventListener("click", () => {
    if (type === "add") {
      ...
      console.log(
        `Comparing originalProject.name: "${originalProject.name}" with newProjectName: "${newProjectName}"`,
      );
      if (originalProject.name !== newProjectName) {
        // Remove the todo from the original project
        console.log(
          "Before removing todo from originalProject:",
          originalProject.todoList,
        );
        originalProject.todoList = originalProject.todoList.filter(
          (t) => t.id !== todo.id,
        );
        console.log(
          "After removing todo from originalProject:",
          originalProject.todoList,
        );
        ...
        const todoIndex = originalProject.todoList.findIndex(
          (t) => t.id === todo.id,
        );
...
function deleteProject(projectId) {
  ...
  if (projectId) {
    currentProjectIndex = currentData.findIndex(
...
function deleteProjectConfirmationPopup(projectId, projectName) {
  console.log(
    `deleteProjectConfirmationPopup(projectId)launched with ID: ${projectId}`,
  );
  // Add the overlay window
...
function deleteTodoConfirmationPopup(
  todoId,
  todoName,
  currentProject,
  from = "",
  ...
  console.log(
    `deleteTodoConfirmationPopup(projectId)launched with ID: ${todoId}`,
  );
  // Add the overlay window
  const confirmationPopupOverlay = document.createElement("div");
  confirmationPopupOverlay.classList.add("confirmation_popup_overlay");
  ...
  confirmationConfirmButton.addEventListener("click", () => {
    ...
    if (currentState === "dynamic") {
      ...
      console.log(
        `Attempting to delete todo with ID: (${todoId}) from dynamic state`,
      );
      ...
      console.log(
        `Attempting to delete todo with ID: (${todoId}) from ${currentState} state`,
      );
      ...
      if (projectFound) {
        ...
        console.error(
          `Error while attempting to delete todo with ID ${todoId}`,
        );
      }
    }
  });
